...
function loadModel() {
    ...
    try {
        ...
        const modelURL = 'https://teachablemachine.withgoogle.com/models/zXbY7gBDr/';
...
// เงื่อนไขสำหรับกรณีที่ไม่มีฟอสฟอรัสหรือไม่พบค่าฟอสฟอรัส
if (parsedTopResult.label.toLowerCase().includes("ไม่มี") || 
    parsedTopResult.label.toLowerCase().includes("ไม่พบ") || 
    parsedTopResult.value === null) {
    resultHTML += 'คำแนะนำ: ไม่พบฟอสฟอรัสในดิน ควรเพิ่มปุ๋ยฟอสฟอรัสเพื่อปรับปรุงคุณภาพดิน';
} else if (parsedTopResult.label.toLowerCase().includes("ต่ำ") || 
    (parsedTopResult.value !== null && parsedTopResult.value < 20)) {
    resultHTML += 'คำแนะนำ: ควรเพิ่มปุ๋ยฟอสฟอรัสให้กับดิน';
} else if (parsedTopResult.label.toLowerCase().includes("สูง") || 
          (parsedTopResult.value !== null && parsedTopResult.value > 60)) {
    resultHTML += 'คำแนะนำ: ระดับฟอสฟอรัสเพียงพอแล้ว ไม่จำเป็นต้องเพิ่มปุ๋ย';
} else {
    resultHTML += 'คำแนะนำ: ระดับฟอสฟอรัสปานกลาง อาจเพิ่มปุ๋ยได้เล็กน้อย';
}
resultHTML += '</div>';
